 ...
 * 
 */
@Name("recentlyAddedOrModifiedPublishedContentObjectsDashboardItem")
@Scope(ScopeType.CONVERSATION)
public class RecentlyAddedOrModifiedPublishedContentObjectsDashboardItem extends ContentObjectListBean{
	...
	public ContentObjectDataModel getReturnedContentObjects() {
		if (super.getReturnedContentObjects() == null){
			...
			try {
				...
				/*
				// We add the date into search criteria
				DateRangeCriteria dateRangeCriteria = new DateRangeCriteria();
				dateRangeCriteria.setDateType(DateType.Modified);
				dateRangeCriteria.setFrom(searchBackDate);
				dateRangeCriteria.setTo(Calendar.getInstance());
				List dateRangeCriteriaList = new ArrayList<DateRangeCriteria>();
				dateRangeCriteriaList.add(dateRangeCriteria);
				getContentObjectCriteria().setDateCriteria(dateRangeCriteriaList);
				 */


				/*
				 * we are looking for content objects modified the last <numberOfDaysToSearchBackForRecentlyAddedOrModifiedPublishedContentObjects>
				 */
