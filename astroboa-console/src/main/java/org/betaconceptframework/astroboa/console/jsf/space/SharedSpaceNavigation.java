 ...
 * 
 */
public class SharedSpaceNavigation extends AbstractUIBean {
	...
	@Out(required=false)
	private List<Space> breadCrumbForSharedSpaces ;
	...
	@Out(required=false)
	private List<SpaceItem> sharedSpaceItems;
	
	private CmsRepositoryEntityFactory cmsRepositoryEntityFactory;
	
	public String showSharedSpace_UIAction(){
		
		return showSpace_UIAction(spaceService.getOrganizationSpace());
	}
	...
	public String showSpace_UIAction(Space space) {
		...
		return pageController.loadPageComponentInDynamicUIArea(DynamicUIAreaPageComponent.SHARED_SPACE_NAVIGATOR.getDynamicUIAreaPageComponent());
	}
	...
	public void getBreadCrumbForSharedSpaces(){
		if (currentSpace == null)
			currentSpace = spaceService.getOrganizationSpace();

		createBreadCrumbSpaces();
	}
	
	private void createBreadCrumbSpaces() {
		breadCrumbForSharedSpaces = new ArrayList<Space>();
		
		breadCrumbForSharedSpaces.add(currentSpace);
		
		Space tempSpace = currentSpace.getParent();
		
		while(tempSpace != null){
			breadCrumbForSharedSpaces.add(tempSpace);
			tempSpace = tempSpace.getParent();
		}
		
		Collections.reverse(breadCrumbForSharedSpaces);
	}
	
	@Factory("sharedSpaceItems")
	public void getSharedSpaceItems(){
		if (currentSpace == null)
			currentSpace = spaceService.getOrganizationSpace();
	...
	private void sortSpaceItems() {
		Collections.sort(sharedSpaceItems, spaceItemLocalizeLabelComparator);
	}
	...
	private void addNewSpaceItemForSpace(Space childSpace) {
		...
		sharedSpaceItems.add(spaceItem);
	}
	...
	public void deleteSpace(){
		...
		if (StringUtils.isNotBlank(selectedSpaceId)) {
			try {
				...
				for (SpaceItem spaceItem : sharedSpaceItems) {
	...
	public void handleDraggedAndDroppedItem_Listener(DropEvent dropEvent) {
		...
		else if (dropEvent.getDragType().equals("sharedSpace")) {
