...
bcmslib.jQuery.bt = {version: '0.9.5-rc1'};
...
;(function($) { 
  ...
  bcmslib.jQuery.fn.bt = function(content, options) {
    ...
    if (bcmslib.jQuery.fn.hoverIntent && bcmslib.jQuery.bt.defaults.trigger == 'hover') {
      bcmslib.jQuery.bt.defaults.trigger = 'hoverIntent';
    }
  
    return this.each(function(index) {
  
      var opts = bcmslib.jQuery.extend(false, bcmslib.jQuery.bt.defaults, bcmslib.jQuery.bt.options, options);
      ...
      if (opts.killTitle) {
        bcmslib.jQuery(this).find('[title]').andSelf().each(function() {
          if (!bcmslib.jQuery(this).attr('bt-xTitle')) {
            bcmslib.jQuery(this).attr('bt-xTitle', bcmslib.jQuery(this).attr('title')).attr('title', '');
          }
        });
      }    
      ...
      if (opts.trigger[0] == 'hoverIntent') {
        var hoverOpts = bcmslib.jQuery.extend(opts.hoverIntentOpts, {
          over: function() {
            this.btOn();
          },
          out: function() {
        ...
        bcmslib.jQuery(this).hoverIntent(hoverOpts);
      
      }
      else if (opts.trigger[0] == 'hover') {
        bcmslib.jQuery(this).hover(
          function() {
            this.btOn();
          },
          function() {
        ...
        if (bcmslib.jQuery(this).hasClass('bt-active')) {
          this.btOff();
        }
        else {
          this.btOn();
        ...
        // bcmslib.jQuery('#selector').btOn();
        // bcmslib.jQuery('#selector').btOff();
      }
        ...
        bcmslib.jQuery(this).bind(opts.trigger[0], function() {
          if (bcmslib.jQuery(this).hasClass('bt-active')) {
            this.btOff();
          }
          else {
       ...
       */
      this.btOn = function () {
        if (typeof bcmslib.jQuery(this).data('bt-box') == 'object') {
        ...
        // turn off other tips
        bcmslib.jQuery(bcmslib.jQuery.bt.vars.closeWhenOpenStack).btOff();
        
        // add the class to the target element (for hilighting, for example)
        // bt-active is always applied to all, but activeClass can apply another
        bcmslib.jQuery(this).addClass('bt-active ' + opts.activeClass);
  
        if (contentSelect && opts.ajaxPath == null) {
          // bizarre, I know
          if (opts.killTitle) {
            // if we've killed the title attribute, it's been stored in 'bt-xTitle' so get it..
            bcmslib.jQuery(this).attr('title', bcmslib.jQuery(this).attr('bt-xTitle'));
          }
          // then evaluate the selector... title is now in place
          content = bcmslib.jQuery.isFunction(opts.contentSelector) ? opts.contentSelector.apply(this) : eval(opts.contentSelector);
          if (opts.killTitle) {
            // now remove the title again, so we don't get double tips
            bcmslib.jQuery(this).attr('title', '');
          }
        }
        ...
        if (opts.ajaxPath != null && content == false) {
          ...
          var cacheData = opts.ajaxCache ? bcmslib.jQuery(document.body).data('btCache-' + url.replace(/\./g, '')) : null;
          if (typeof cacheData == 'string') {
            content = selector ? bcmslib.jQuery("<div/>").append(cacheData.replace(/<script(.|\s)*?\/script>/g, "")).find(selector) : cacheData;
          }
          else {
            var target = this;
                      
            // set up the options
            var ajaxOpts = bcmslib.jQuery.extend(false,
            {
              type: opts.ajaxType,
              data: opts.ajaxData,
              ...
              complete: function(XMLHttpRequest, textStatus) {
                if (textStatus == 'success' || textStatus == 'notmodified') {
                  if (opts.ajaxCache) {
                    bcmslib.jQuery(document.body).data('btCache-' + url.replace(/\./g, ''), XMLHttpRequest.responseText);
                  }
                  ajaxTimeout = false;
                  content = selector ?
                ...
                if (bcmslib.jQuery(target).hasClass('bt-active')) {
                  target.btOn();
                }
              }
            }, opts.ajaxOpts);
            // do the ajax request
            bcmslib.jQuery.ajax(ajaxOpts);
        ...
        if (opts.shadow && !shadowSupport()) {
          ...
          bcmslib.jQuery.extend(opts, opts.noShadowOpts);
        }
        ...
        if (opts.offsetParent){
          ...
          var offsetParent = bcmslib.jQuery(opts.offsetParent);
          var offsetParentPos = offsetParent.offset();
          var pos = bcmslib.jQuery(this).offset();
          var top = numb(pos.top) - numb(offsetParentPos.top) + numb(bcmslib.jQuery(this).css('margin-top')) - shadowShiftY; // IE can return 'auto' for margins
          var left = numb(pos.left) - numb(offsetParentPos.left) + numb(bcmslib.jQuery(this).css('margin-left')) - shadowShiftX;
        }
        else {
          // if the target element is absolutely positioned, use its parent's offsetParent instead of its own
          var offsetParent = (bcmslib.jQuery(this).css('position') == 'absolute') ? bcmslib.jQuery(this).parents().eq(0).offsetParent() : bcmslib.jQuery(this).offsetParent();
          var pos = bcmslib.jQuery(this).btPosition();
          var top = numb(pos.top) + numb(bcmslib.jQuery(this).css('margin-top')) - shadowShiftY; // IE can return 'auto' for margins
          var left = numb(pos.left) + numb(bcmslib.jQuery(this).css('margin-left')) - shadowShiftX;
        }

        var width = bcmslib.jQuery(this).btOuterWidth();
        var height = bcmslib.jQuery(this).outerHeight();
        ...
        if (typeof content == 'object') {
          ...
          var clone = bcmslib.jQuery(original).clone(true).show();
          // also store a reference to the original object in the clone data
          // and a reference to the clone in the original
          var origClones = bcmslib.jQuery(original).data('bt-clones') || [];
          origClones.push(clone);
          bcmslib.jQuery(original).data('bt-clones', origClones);
          bcmslib.jQuery(clone).data('bt-orig', original);
          bcmslib.jQuery(this).data('bt-content-orig', {original: original, clone: clone});
          content = clone;
        }
        ...
        var $text = bcmslib.jQuery('<div class="bt-content"></div>').append(content).css({padding: opts.padding, position: 'absolute', width: (opts.shrinkToFit ? 'auto' : opts.width), zIndex: opts.textzIndex, left: shadowShiftX, top: shadowShiftY}).css(opts.cssStyles);
        // create the wrapping box which contains text and canvas
        // put the content in it, style it, and append it to the same offset parent as the target
        var $box = bcmslib.jQuery('<div class="bt-wrapper"></div>').append($text).addClass(opts.cssClass).css({position: 'absolute', width: opts.width, zIndex: opts.wrapperzIndex, visibility:'hidden'}).appendTo(offsetParent);
        
        // use bgiframe to get around z-index problems in IE6
        // http://plugins.bcmslib.jQuery.com/project/bgiframe
        if (bcmslib.jQuery.fn.bgiframe) {
          $text.bgiframe();
          $box.bgiframe();  
        }
  
        bcmslib.jQuery(this).data('bt-box', $box);
  
        // see if the text box will fit in the various positions
        var scrollTop = numb(bcmslib.jQuery(document).scrollTop());
        var scrollLeft = numb(bcmslib.jQuery(document).scrollLeft()); 
        var docWidth = numb(bcmslib.jQuery(window).width());
        var docHeight = numb(bcmslib.jQuery(window).height());
        var winRight = scrollLeft + docWidth;
        var winBottom = scrollTop + docHeight;
        var space = new Object();
        var thisOffset = bcmslib.jQuery(this).offset();
        ...
        bcmslib.jQuery(canvas).attr('width', (numb($text.btOuterWidth(true)) + opts.strokeWidth*2 + shadowMarginX)).attr('height', (numb($text.outerHeight(true)) + opts.strokeWidth*2 + shadowMarginY)).appendTo($box).css({position: 'absolute', zIndex: opts.boxzIndex});
        ...
        if (opts.overlay) {
          ...
          var overlay = bcmslib.jQuery('<div class="bt-overlay"></div>').css({
              position: 'absolute',
          ...
          bcmslib.jQuery(this).data('overlay', overlay);
        }
        ...
        if (opts.clickAnywhereToClose) {
          bcmslib.jQuery.bt.vars.clickAnywhereStack.push(this);
          bcmslib.jQuery(document).click(bcmslib.jQuery.bt.docClick);
        }
        
        // stick this element into the closeWhenOthersOpen stack
        if (opts.closeWhenOthersOpen) {
          bcmslib.jQuery.bt.vars.closeWhenOpenStack.push(this);
        }
      ...
      this.btOff = function() {
        ...
        var box = bcmslib.jQuery(this).data('bt-box');
        ...
        i.btCleanup = function(){
          var box = bcmslib.jQuery(i).data('bt-box');
          var contentOrig = bcmslib.jQuery(i).data('bt-content-orig');
          var overlay = bcmslib.jQuery(i).data('bt-overlay');
          if (typeof box == 'object') {
            bcmslib.jQuery(box).remove();
            bcmslib.jQuery(i).removeData('bt-box');
          }
          if (typeof contentOrig == 'object') {
            var clones = bcmslib.jQuery(contentOrig.original).data('bt-clones');
            bcmslib.jQuery(contentOrig).data('bt-clones', arrayRemove(clones, contentOrig.clone));        
          }
          if (typeof overlay == 'object') {
            bcmslib.jQuery(overlay).remove();
            bcmslib.jQuery(i).removeData('bt-overlay');
          }
          
          // remove this from the stacks
          bcmslib.jQuery.bt.vars.clickAnywhereStack = arrayRemove(bcmslib.jQuery.bt.vars.clickAnywhereStack, i);
          bcmslib.jQuery.bt.vars.closeWhenOpenStack = arrayRemove(bcmslib.jQuery.bt.vars.closeWhenOpenStack, i);
          
          // remove the 'bt-active' and activeClass classes from target
          bcmslib.jQuery(i).removeClass('bt-active ' + opts.activeClass);
  ...
  }; // </ bcmslib.jQuery.fn.bt() >
  
  /**
   * bcmslib.jQuery's compat.js (used in Drupal's bcmslib.jQuery upgrade module, overrides the $().position() function
   *  this is a copy of that function to allow the plugin to work when compat.js is present
   *  once compat.js is fixed to not override existing functions, this function can be removed
   *  and .btPosion() can be replaced with .position() above...
   */
  bcmslib.jQuery.fn.btPosition = function() {
  
    function num(elem, prop) {
      return elem[0] && parseInt( bcmslib.jQuery.curCSS(elem[0], prop, true), 10 ) || 0;
    };
    ...
    return results;
  }; // </ bcmslib.jQuery.fn.btPosition() >
  
  
  /**
  * bcmslib.jQuery's dimensions.js overrides the $().btOuterWidth() function
  *  this is a copy of original bcmslib.jQuery's outerWidth() function to 
  *  allow the plugin to work when dimensions.js is present
  */
  bcmslib.jQuery.fn.btOuterWidth = function(margin) {
  ...
  }; // </ bcmslib.jQuery.fn.btOuterWidth() >
  
  /**
   * A convenience function to run btOn() (if available)
   * for each selected item
   */
  bcmslib.jQuery.fn.btOn = function() {
    return this.each(function(index){
      if (bcmslib.jQuery.isFunction(this.btOn)) {
        this.btOn();
      }
    });
  }; // </ $().btOn() >
   ...
   * for each selected item
   */
  bcmslib.jQuery.fn.btOff = function() {
    return this.each(function(index){
      if (bcmslib.jQuery.isFunction(this.btOff)) {
        this.btOff();
      }
    });
  }; // </ $().btOff() >
  
  bcmslib.jQuery.bt.vars = {clickAnywhereStack: [], closeWhenOpenStack: []};
  
  /**
   * This function gets bound to the document's click event
   * It turns off all of the tips in the click-anywhere-to-close stack
   */
  bcmslib.jQuery.bt.docClick = function(e) {
    if (!e) {
      var e = window.event;
    };
    // if clicked element is a child of neither a tip NOR a target
    // and there are tips in the stack
    if (!bcmslib.jQuery(e.target).parents().andSelf().filter('.bt-wrapper, .bt-active').length && bcmslib.jQuery.bt.vars.clickAnywhereStack.length) {
      // if clicked element isn't inside tip, close tips in stack
      bcmslib.jQuery(bcmslib.jQuery.bt.vars.clickAnywhereStack).btOff();
      bcmslib.jQuery(document).unbind('click', bcmslib.jQuery.bt.docClick);
    }
  }; // </ docClick() >
   ...
   *
   *   bcmslib.jQuery.bt.options.width = 400;
   *
   * Be sure to use *bcmslib.jQuery.bt.options* and not bcmslib.jQuery.bt.defaults when overriding
   *
  ...
  bcmslib.jQuery.bt.defaults = {
    ...
    contentSelector:  "bcmslib.jQuery(this).attr('title')", // if there is no content argument, use this selector to retrieve the title
                                             // a function which returns the content may also be passed here
  
    ajaxPath:         null,                  // if using ajax request for content, this contains url and (opt) selector
                                             // this will override content and contentSelector
                                             // examples (see bcmslib.jQuery load() function):
                                             //   '/demo.html'
                                             //   '/help/ajax/snip'
                                             //   '/help/existing/full div#content'
                                             
                                             // ajaxPath can also be defined as an array
                                             // in which case, the first value will be parsed as a bcmslib.jQuery selector
    ...
    ajaxOpts:         {},                    // any other ajax options - timeout, passwords, processing functions, etc...
                                             // see http://docs.bcmslib.jQuery.com/Ajax/bcmslib.jQuery.ajax#options
    ...
    hoverIntentOpts:  {                          // options for hoverIntent (if installed)
                        interval: 300,           // http://cherne.net/brian/resources/bcmslib.jQuery.hoverIntent.html
                        timeout: 500
                      }
                                                 
  }; // </ bcmslib.jQuery.bt.defaults >
  
  bcmslib.jQuery.bt.options = {};
