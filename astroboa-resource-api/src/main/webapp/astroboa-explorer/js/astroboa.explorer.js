...
(function($){
	...
	$.fn.astroboaExplorer = function(configuration) {
		...
		return this.each(function() {
			...
			selectedDateValue, //this is the selected date in the format recognized by the Astroboa Query Engine 
			...
			idOfCriterionButtonsDiv = "criterionButtonDiv",
			...
			//Various messages
			message = { 
					repositorySelectorLabel : {el:"Αποθήκη Περιεχομένου :", en:"Repository :"},
			...
			operatorOptionsPerType = {  
					String : [operator.eq, operator.ne,operator.nullOp, operator.notNull],
			...
			inputComponentPerType = {
				...
				function initContentTab(){
					...
					if (elementExists(idOfCriterionBuilderDiv)){
						$('#'+idOfCriterionBuilderDiv).empty();
					}
					else{
						$('#'+idOfContentTab).append('<div id="'+idOfCriterionBuilderDiv+'"></div>');
					}
				...
				function buildContentTypeSelector(){
					...
					if (! elementExists(contentTypeSelectorName)){
						$('#'+idOfCriterionBuilderDiv).append('<div id="'+idOfContentTypeSelectorDiv+'" class="'+css.selectorDiv+'"></div>');
						...
						dataType : 'json',
					 		...
					 		success : function(data){
					 			...
					 			$.each(data.contentType, function(i, contentType){
					 				...
					 				var label=retrieveLabel(contentType.label, configuration.locale);
				...
				function buildPropertySelector(){
					...
					if (! elementExists(propertySelectorName)){
						$('#'+idOfCriterionBuilderDiv).append('<div id="'+idOfPropertySelectorDiv+'" class="'+css.selectorDiv+'"></div>');
				...
				function buildOptionsForComplexProperty(complexProperty, cssClass){
					...
					var label=retrieveLabel(complexProperty.label, configuration.locale);
					
					var options = '<optgroup label="'+label+'" class="'+cssClass+'">';
					
					$.each(complexProperty.propertyList, function(i, property){

		 				var label=retrieveLabel(property.label, configuration.locale);
				...
				function buildOperatorSelector(){
					...
					if (propertySelectedValue != defaultValueForSelector){
						...
						if (! elementExists(operatorSelectorName)){
							$('#'+idOfCriterionBuilderDiv).append('<div id="'+idOfOperatorSelectorDiv+'" class="'+css.selectorDiv+'"></div>');
				...
				function buildRestrictionComponent(){
					...
					if (operatorSelectedValue != defaultValueForSelector){
						...
						if (! elementExists(idOfRestrictionValueComponent)){
							$('#'+idOfCriterionBuilderDiv).append('<div id="'+idOfRestrictionDiv+'" class="'+css.selectorDiv+'"></div>');
						}
						else{
				...
				function buildCriterionActions(){
					...
					if (propertySelectedValue != defaultValueForSelector){
						...
						if (! elementExists(idOfCriterionButtonsDiv)){
							$('#'+idOfCriterionBuilderDiv).append('<div id="'+idOfCriterionButtonsDiv+'" class="'+css.selectorDiv+'"></div>');
						}
						else{
							$('#'+idOfCriterionButtonsDiv).empty();
						}
					
						$('#'+idOfCriterionButtonsDiv).append('<div id="'+idOfButtonForAddingNewCriterion+'">'+message.addNewCriterion[configuration.locale]+'</div>');
						...
						if (elementExists(idOfCriterionButtonsDiv)){
							$('#'+idOfCriterionButtonsDiv).remove();
						}
					}
									...
									$.ajax({
										...
										success: function( data ) {
											...
											if (data.totalResourceCount > 0){
												$.each( data.resourceCollection.resource, function(index,  resource ) {
													suggestionData.push({
																		label: retrieveLabelForTopic(resource, configuration.locale),
																		value: resource.name,
																		category : category
																	});
												});
					...
					$( '#'+idOfRestrictionValueComponent ).datepicker(
							...
							{ 
								...
								onSelect: function(dateText, inst) {
									...
									selectedDateValue = $.datepicker.formatDate('yy-mm-dd', date);
								}
							}
					);
